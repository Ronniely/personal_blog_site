      ...
      <n-grid x-gap="15" y-gap="15" cols="1 s:2 m:3" responsive="screen">
        <n-grid-item v-for="article of articleList" :key="article.id" class="article-item">
          <div class="article-cover">
            <router-link :to="`/article/${article.id}`"><img v-lazy="article.article_cover" class="cover" />
            </router-link>
          </div>
...
function getTagArticleList() {
  ...
  ArticleAPI.findArticleClassifyTagApi(data).then((res: any) => {
...
function getCategoryArticleList() {
  ...
  ArticleAPI.findArticleClassifyCategoryApi(data).then((res: any) => {
    articleList.value = res.data.list;
  });
}
