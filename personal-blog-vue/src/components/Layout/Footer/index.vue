<template>
  <footer v-if="!isMessage" class="footer-wrapper">
    ...
    <p>
      ©
      {{ formatDate(blogStore.blogInfo.website_config?.website_info?.website_create_time || '2020', "YYYY") }} -
      {{ new Date().getFullYear() }} By
      {{ blogStore.blogInfo.website_config?.website_info?.website_author || '作者' }}
    </p>
    <a href="https://beian.miit.gov.cn/" target="_blank">
      {{ blogStore.blogInfo.website_config?.website_info?.website_record_no || '备案号' }}
    </a>
  </footer>
</template>
