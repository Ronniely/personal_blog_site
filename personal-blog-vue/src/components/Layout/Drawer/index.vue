<template>
  <n-drawer v-model:show="drawerVisible" class="side-bg" :width="240" :block-scroll="false" placement="right">
    <n-drawer-content style="padding-top: 0.5rem">
      <div class="author-container">
        <img class="author-avatar"
          :src="blogStore.blogInfo.website_config?.website_info?.website_avatar || '/default-avatar.png'" />
        <p class="author-name">
          {{ blogStore.blogInfo.website_config?.website_info?.website_author || '作者' }}
        </p>
        <div class="site-desc">
          {{ blogStore.blogInfo.website_config?.website_info?.website_intro || '网站介绍' }}
        </div>
      </div>
      <blog-info></blog-info>
      ...
      <ul class="side-menu">
        <template v-for="menu of menuList" :key="menu.name">
          ...
          <li v-else class="item dropdown" :class="{ expand: expand(menu.children) }">
            ...
            <ul class="submenu">
              <li v-for="submenu of menu.children" :key="submenu.name" class="item"
